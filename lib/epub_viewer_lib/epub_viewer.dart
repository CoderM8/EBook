...
class EpubViewer {
  ...
  static const EventChannel _pageChannel =
      const EventChannel('page');
  ...
  static void setConfig(
      {MaterialColor? themeColorMaterial,
      String identifier = 'book',
      bool nightMode = false,
      EpubScrollDirection scrollDirection =
          EpubScrollDirection.ALLDIRECTIONS,
      bool allowSharing = false,
      bool enableTts = false}) async {
    Color? themeColor;
    if (themeColorMaterial != null) {
      themeColor = themeColorMaterial[500]; // Use the primary shade of the MaterialColor
    }
  ...
  static Future openAsset(String bookPath,
    ...
    if (extension(bookPath) == '.epub') {
      Map<String, dynamic> agrs = {
        "bookPath":
            (await Util.getFileFromAsset(bookPath)).path,
        'lastLocation': lastLocation == null
            ? ''
            : jsonEncode(lastLocation.toJson()),
      };
